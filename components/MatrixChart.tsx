    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border border-gray-200 shadow-lg rounded-md text-sm">
          ...
          <p className="text-blue-600">Importance: {d.y.toFixed(1)}</p>
          <p className="text-green-600">Ease: {d.x.toFixed(1)}</p>
        </div>
      );
    }
    return null;
      ...
      <ResponsiveContainer width="100%" height="100%">
        <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
          <XAxis 
            ...
            domain={[0, 10]} 
            tickCount={6}
            label={{ value: 'Easiness (Hard → Easy)', position: 'bottom', offset: 0 }}
          />
          <YAxis 
            type="number" 
            dataKey="y" 
            ...
            domain={[0, 10]} 
            tickCount={6}
            label={{ value: 'Importance (Low → High)', angle: -90, position: 'left' }}
          />
